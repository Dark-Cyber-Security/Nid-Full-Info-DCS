...
if(isset($_GET['nid']) && isset($_GET['dob'])) {
    ...
    if (isset($token)) {
        ...
        echo json_encode(json_decode($secondResponse), JSON_PRETTY_PRINT);
    } else {
        header('Content-Type: application/json; charset=utf-8');
        echo json_encode(array("success" => false, "message" => "I don't know. Have error? Contact https://t.me/Owner_Of_DCS"), JSON_PRETTY_PRINT);
        error_log('Access token not present in the response.');
    }
} else {
    echo "Please provide 'nid' and 'dob' parameters.";
}
?>
